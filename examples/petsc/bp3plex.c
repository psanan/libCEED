...
/// @file
/// Diff operator example using PETSc
...
static PetscErrorCode MatMult_Diff(Mat A, Vec X, Vec Y) {
  ...
  //CeedVectorSetArray(user->xceed, CEED_MEM_HOST, CEED_COPY_VALUES, (const PetscScalar**)&x);
  //CeedVectorSetArray(user->xceed, CEED_MEM_HOST, CEED_COPY_VALUES, (PetscScalar*)y);
...
/*
  CeedVectorGetArray(user->yceed, CEED_MEM_HOST, &ty);
  CeedVectorGetArrayRead(user->xceed, CEED_MEM_HOST, &tx);
 
  CeedInt     nt=user->lsize;  
  printf("nt %d\n",nt);
  for (CeedInt i=0; i<nt; i++) {
    printf("user->xceed[%d] is %f, and length is %d \n",i,tx[i],nt);
    printf("x[%d] is %f, and length is %d \n",i,x[i],nt); 
    printf("user->yceed[%d] is %f, and length is %d \n",i,ty[i],nt);
    printf("y[%d] is %f, and length is %d \n",i,y[i],nt);
  }
  
  ierr = CeedVectorRestoreArrayRead(user->xceed, &tx); CHKERRQ(ierr);
  ierr = CeedVectorRestoreArray(user->yceed, &ty); CHKERRQ(ierr);
*/
...
int main(int argc, char **argv) {
  ...
  PetscInt degree, qextra, dim = 3, melem[3] = {1, 1, 1}, lsize, gsize;
  PetscScalar *r;
  ...
  //filename="3Dbrick4els.exo";
   read_mesh=PETSC_TRUE;
  if (read_mesh) {
  ...
  //ierr = PetscSpaceGetDegree(sp, &testd, NULL);CHKERRQ(ierr);
  //printf("degree test %d\n",testd);  
  ...
  CeedQFunctionCreateInterior(ceed, 1, SetupDiff, __FILE__ ":SetupDiff", &qf_setup);
  ...
  CeedQFunctionCreateInterior(ceed, 1, Diff, __FILE__ ":Diff", &qf_diff);
  ...
  CeedQFunctionCreateInterior(ceed, 1, Error, __FILE__ ":Error", &qf_error);
  ...
  printf("Length of rho is %d, length of xcoord %d, gsize %d, Xlocsize %d, lsize %d \n",ntestt, ntest,gsize,Xlocsize,lsize);
...
/*
  Vec   ref, wrk_vec, vec_rhs;
  PetscScalar vareps;
  PetscInt i;   
  PetscInt its=0;
  
  ierr = VecDuplicate(X,&wrk_vec);CHKERRQ(ierr);
  ierr = VecDuplicate(X,&vec_rhs);CHKERRQ(ierr);
  ierr = VecDuplicate(X,&ref);CHKERRQ(ierr);
  ...
  vareps = 1.0;
    for (i=0; i<lsize; i++) 
     {its=its+1;
      ierr = VecCopy(vec_rhs,wrk_vec); CHKERRQ(ierr); //reset J(eps) for each point
      ierr = VecZeroEntries(wrk_vec);
      VecSetValue(wrk_vec,i, vareps,ADD_VALUES);
      MatMult_Diff(mat, wrk_vec, vec_rhs);
      ierr = PetscViewerASCIIOpen(PETSC_COMM_WORLD,"matbp3.m",&viewfile);CHKERRQ(ierr);
    ierr = PetscViewerPushFormat(viewfile,PETSC_VIEWER_ASCII_MATLAB);CHKERRQ(ierr);
    PetscSNPrintf(var,sizeof(var),"mat(:,%d)",its);
    ierr = PetscObjectSetName((PetscObject)vec_rhs,var);
    ierr = VecView(vec_rhs,viewfile);CHKERRQ(ierr);
    //VecView(wrk_vec,PETSC_VIEWER_STDOUT_WORLD); 
    //VecView(..,PETSC_VIEWER_STDOUT_WORLD); 
